...
<body class="bg-black">
    ...
    <main class="mt-10 mb-20 divide-y max-w-2xl mx-auto p-4 space-y-3">
        <div class="px-4 py-5 duration-150 border-2 shadow-xl shadow-lime-600 border-lime-600 rounded-xl wow animate__animated animate__zoomIn">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="grid max-w-2xl mx-auto mt-8">
                        <div class="flex flex-col items-center space-y-5 sm:flex-row sm:space-y-0">
                            ...
                            echo '<img id="cimg" class="object-cover w-40 h-40 p-1 rounded-full ring-2 ring-lime-600" src="' . $imagePath . '" alt="User Image">';
                            ?>
                            <div class="flex flex-col space-y-5 sm:ml-8">
                                <label for="dropzone-file" class="py-3.5 px-7 text-base cursor-pointer font-medium text-lime-100 focus:outline-none bg-lime-700 rounded-lg border border-lime-200 hover:bg-lime-600 focus:z-10 focus:ring-4 focus:ring-lime-200">
                                    <input id="dropzone-file" type="file" name="file-upload" onchange="displayImg(this)" class="hidden" />
                                    Change Image
                                </label>

                                <button type="button" onclick="deleteImg()" class="py-3.5 px-7 text-base font-medium text-lime-700 focus:outline-none bg-white rounded-lg border border-lime-200 hover:bg-lime-100 hover:text-[#202142] focus:z-10 focus:ring-4 focus:ring-lime-200">
                                    Delete Image
                                </button>
                            </div>
                        </div>
                        <div class="items-center mt-4 sm:mt-14 text-[#202142]">
                            ...
                            <div class="flex flex-col items-center w-full my-2 space-x-0 space-y-2 sm:flex-row sm:space-x-4 sm:space-y-0 sm:mb-6">
                                <div class="w-full">
                                    <label for="first_name" class="block mb-2 text-sm font-medium text-lime-600">Full Name</label>
                                    <input type="text" id="first_name" name="full_name" class="bg-indigo-50 border border-lime-300 text-black text-sm rounded-lg block w-full p-2.5 " placeholder="Your first name" value="<?php echo $_SESSION['full_name']; ?>">
                                </div>

                                <div class="w-full">
                                    <label for="email" class="block mb-2 text-sm font-medium text-lime-600">Email</label>
                                    <input type="email" id="email" name="email" class="bg-indigo-50 border border-lime-300 text-black text-sm rounded-lg block w-full p-2.5 " placeholder="your.email@mail.com" value="<?php echo $_SESSION['email']; ?>">
                                </div>
                            ...
                            <div class="flex flex-col items-center w-full mb-2 space-x-0 space-y-2 sm:flex-row sm:space-x-4 sm:space-y-0 sm:mb-6">
                                <div class="w-full">
                                    <div class='flex flex-col text-gray-600 py-2' x-data="{ show: true }">
                                        <div class="flex flex-col mb-3">
                                            <label for="password" class="block mb-2 text-sm font-medium text-lime-600">Password</label>
                                            <div class="relative mt-2">
                                                <!-- Password input field -->
                                                <input class="bg-indigo-50 border border-lime-300 text-black text-sm rounded-lg block w-full p-2.5" :type="show ? 'password' : 'text'" id="password" name="password" autocomplete="current-password" placeholder="Enter your password" />
                                ...
                                <div class="w-full">
                                    <div class='flex flex-col py-2' x-data="{ showConfirm: true }">
                                        ...
                                        <div class="flex flex-col mb-3">
                                            <label for="confirm_password" class="block mb-2 text-sm font-medium text-lime-600">Confirm Password</label>
                                            <div class="relative mt-2">
                                                <!-- Confirm password input field with show/hide functionality -->
                                                <input class="bg-indigo-50 border border-lime-300 text-black text-sm rounded-lg block w-full p-2.5" :type="showConfirm ? 'password' : 'text'" id="confirm_password" name="confirm_password" autocomplete="new-confirm_password" placeholder="Repeat password" />
                                                <!-- Toggle icons for showing/hiding password -->
                                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5">
                                                    <!-- Eye icon when password is hidden (showConfirm=true) -->
                                                    <svg x-show="showConfirm" class="h-6 text-lime-600" fill="none" @click="showConfirm = !showConfirm" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
                                                        <path fill="currentColor" d="M572.52 241.4C518.29 135.59 410.93 64 288 64S57.68 135.64 3.48 241.41a32.35 32.35 0 0 0 0 29.19C57.71 376.41 165.07 448 288 448s230.32-71.64 284.52-177.41a32.35 32.35 0 0 0 0-29.19zM288 400a144 144 0 1 1 144-144 143.93 143.93 0 0 1-144 144zm0-240a95.31 95.31 0 0 0-25.31 3.79 47.85 47.85 0 0 1-66.9 66.9A95.78 95.78 0 1 0 288 160z"></path>
                                                    </svg>

                                                    <!-- Crossed eye icon when password is shown (showConfirm=false) -->
                                                    <svg x-show="!showConfirm" class="h-6 text-lime-600" fill="none" @click="showConfirm = !showConfirm" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
                                                        <path fill="currentColor" d="M320 400c-75.85 0-137.25-58.71-142.9-133.11L72.2 185.82c-13.79 17.3-26.48 35.59-36.72 55.59a32.35 32.35 0 0 0 0 29.19C89.71 376.41 197.07 448 320 448c26.91 0 52.87-4 77.89-10.46L346 397.39a144.13 144.13 0 0 1-26 2.61zm313.82 58.1l-110.55-85.44a331.25 331.25 0 0 0 81.25-102.07 a32.35 32.35 0 0 0 0-29.19C550.29 135.59 442.93 64 320 64a308.15 308.15 0 0 0-147.32 37.7L45.46 3.37A16 16 0 0 0 23 6.18L3.37 31.45A16 16 0 0 0 6.18 53.9l588.36 454.73a16 16 0 0 0 22.46-2.81l19.64-25.27a16 16 0 0 0-2.82-22.45zm-183.72-142l-39.3-30.38A94.75 94.75 0 0 0 416 256a94.76 94.76 0 0 0-121.31-92.21A47.65 47.65 0 0 1 304 192a46.64 46.64 0 0 1-1.54 10l-73.61-56.89A142.31 142.31 0 0 1 320 112a143.92 143.92 0 0 1 144 144c0 21.63-5.29 41.79-13.9 60.11z"></path>
                                                    </svg>
                                                </div>
                                            </div>
