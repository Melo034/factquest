...
// Check if the token is provided in the GET request
if (!isset($_GET["token"])) {
...
// If the user is not found, terminate the script with an error message
if (!$user) {
    die("token not found");
}
...
<body class="bg-black">
    <section class="text-gray-200">
        ...
        <div class="w-full mx-auto container max-w-screen-xl h-screen relative space-y-8  p-10 lg:px-20 flex flex-col md:flex-row items-center justify-center">
            ...
            <form action="./backend/password_reset.php" method="POST" class="w-full max-w-lg mx-auto md:w-1/2 border border-lime-600 p-10 bg-black shadow-xl shadow-lime-600 border-lime-600 ">
                ...
                <?php endif; ?>
                <input class=' mt-2 p-2' name="token" type="hidden" value="<?= htmlspecialchars($token) ?>" />
                <div class='flex flex-col py-2' x-data="{ show: true }">
